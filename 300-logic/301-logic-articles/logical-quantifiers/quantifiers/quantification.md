...
The quantifiers let us express certain rudimentary claims about the quantity of things that satisfy some condition. Specifically, they allow us to say that all objects satisfy some condition, or that at least one object satisfies some condition. When used in conjunction with identity, they can express more complex numerical claims (e.g. there are exactly three things that satisfy some condition).

* unrestricted universal claim:   `∀xP(x)`
* unrestricted existential claim: `∃xP(x)`
* restricted universal claim:     `∀x(P(x) → Q(x))`
* restricted existential claim:   `∃x(P(x) ⋀ Q(x))`

These sentences are formed out of complex expressions that are not sentences themselves because they contain variables with no associated quantifier. For example, `P(x)` is an (atomic) expression but not a sentence, `P(x) ⋀ Q(x)` is a complex expression but not a sentence; `∀x(P(x) → Q(x))` is a complex sentence. Thus, to systematically describe all the sentences of FOL, we first need to describe a larger class of wffs.
