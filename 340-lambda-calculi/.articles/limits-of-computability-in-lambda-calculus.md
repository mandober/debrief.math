...
The lambda term `(\ x := exp) a` is a reducible expression. The basic reduction step is
...
Note that we use the reduction symbol `~>` in a sloppy manner. `t ~> u` can mean that `t` reduces to `u` in one step or in more steps. In this paper there is no need to distinguish between one step and multistep reduction. In other contexts it might be necessary to use different symbols.
...
3.  The function `lam` expects a Church numeral representing the number `i` and an encoding of the lambda term `a` and returns an encoding of the lambda term `\ xi := a`
...
In order to see that the pairing function is one-to-one and onto we show that given the value of ![[n,m]](https://latex.codecogs.com/png.latex?%5Ctextstyle%20%5Bn%2Cm%5D "[n,m]") we can uniquely recover ![n](https://latex.codecogs.com/png.latex?%5Ctextstyle%20n "n") and ![m](https://latex.codecogs.com/png.latex?%5Ctextstyle%20m "m"). Assume ![z = [n,m]](https://latex.codecogs.com/png.latex?%5Ctextstyle%20z%20%3D%20%5Bn%2Cm%5D "z = [n,m]"). We divide ![z + 1](https://latex.codecogs.com/png.latex?%5Ctextstyle%20z%20%2B%201 "z + 1") by ![2](https://latex.codecogs.com/png.latex?%5Ctextstyle%202 "2") until we get an odd number. ![n](https://latex.codecogs.com/png.latex?%5Ctextstyle%20n "n") is the number of times, we can divide ![z + 1](https://latex.codecogs.com/png.latex?%5Ctextstyle%20z%20%2B%201 "z + 1") by ![2](https://latex.codecogs.com/png.latex?%5Ctextstyle%202 "2"). Since all odd numbers can be uniqueley represented by ![2m + 1](https://latex.codecogs.com/png.latex?%5Ctextstyle%202m%20%2B%201 "2m + 1"), we get a unique value for ![m](https://latex.codecogs.com/png.latex?%5Ctextstyle%20m "m").

For example let ![z = 11](https://latex.codecogs.com/png.latex?%5Ctextstyle%20z%20%3D%2011 "z = 11"). We get ![z + 1 = 12](https://latex.codecogs.com/png.latex?%5Ctextstyle%20z%20%2B%201%20%3D%2012 "z + 1 = 12"). Dividing 12 subsequently by 2 we get the sequence ![6,3](https://latex.codecogs.com/png.latex?%5Ctextstyle%206%2C3 "6,3") i.e. we can divide 2 times by 2 and therefore ![n = 2](https://latex.codecogs.com/png.latex?%5Ctextstyle%20n%20%3D%202 "n = 2"). Resolving the equation ![3 = 2m + 1](https://latex.codecogs.com/png.latex?%5Ctextstyle%203%20%3D%202m%20%2B%201 "3 = 2m + 1") we get ![m = (3 - 1)/ 2 = 1](https://latex.codecogs.com/png.latex?%5Ctextstyle%20m%20%3D%20%283%20-%201%29%2F%202%20%3D%201 "m = (3 - 1)/ 2 = 1").

We can transform the mathematical pairing function ![2^n (2m + 1) - 1](https://latex.codecogs.com/png.latex?%5Ctextstyle%202%5En%20%282m%20%2B%201%29%20-%201 "2^n (2m + 1) - 1") into a lambda term.
...
We still have to define the function `wrap` which given the encoding of a lambda term returns the encoding of the encoding of the the lambda term. In order to achieve that we first encode `zero` and `successor` and then use these encodings to define `wrap`.
...
2.  There is a lambda term `self` which maps the encoding `<a>` of the lambda term `a` into the lambda term `< a <a> >` which represents the encoding of the lambda term `a` applied to its own encoding `<a>`.
